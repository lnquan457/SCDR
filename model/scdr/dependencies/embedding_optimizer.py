...
class EmbeddingOptimizer:
    ...
    def update_old_data_embedding(self, total_embeddings, update_indices, knn_indices,
        ...
        # self.mid_time += time.time() - sta
        # print("mid", self.mid_time)
...
class SkipOptimizer:
    ...
    def _cal_opt_mask(self, update_indices, knn_indices, knn_dists, anchor_positions, row_indices):
        ...
        lower_q, higher_q = np.quantile(other_neighbor_knn_dist, [0.25, 0.75], axis=1)  # 下四分位数
        # higher_q = np.quantile(other_neighbor_knn_dist, 0.75, interpolation='higher', axis=1)  # 上四分位数
